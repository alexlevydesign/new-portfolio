...
(function() {
    // Function to add native lazy loading attribute
    const addNativeLazyLoading = () => {
        // Get all images except navbar logos and LinkedIn icon
        const images = document.querySelectorAll('img:not(.NavBarLogoDesktop):not(.NavBarLogoMobile):not(.LinkedinLogo)');
        ...
        // Handle videos (excluding AutoplayVideo which is handled separately)
        const videos = document.querySelectorAll('video:not(.AutoplayVideo)');
        videos.forEach(video => {
        ...
        images.forEach(img => {
            ...
            if (img.complete) {
                ...
                img.addEventListener('error', () => {
                    img.classList.add('loaded'); // Still show the broken image
